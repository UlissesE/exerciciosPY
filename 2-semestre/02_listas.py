...
def cresce(lista):
    posicao_ultimo_numero = len(lista) - 1
    proximo_numero = lista[posicao_ultimo_numero] + 1
...
def cria_naipe(naipe):
    if naipe not in ["o", "p", "c", "e"]:
        print("Naipe não disponivel. Tente novamente")
        return
...
def acima_da_media(lista):
    ...
    for numero in lista:
        if numero > media_lista:
            lista_acima_da_media.append(numero)
